...
check_camera_af_aging_result() {
  ...
  LOG_FILE="/blackbox/camera/log/lens_drop_step_info.log"
  LOG_STR="Lens step drop happened"
...
camera_service_aging() {
  ...
  if [ $ack_ret -ne 0 ]; then
    cs_exit $E_SET_VF_R "CS_AGING: set 4k 60fps record video format err:$ack_ret, send_cnt:$send_cnt"
  fi
  sleep 2
...
#turn lens PI ON
cam_dt_cmdline -r -e lens-lens_PI_enable enable=1
#enable lens drop step cehck
cam_dt_cmdline -e camdev-lens_drop_check_enable enable=1

cs_log "*****CS_AGING: need to aging for $aging_cnt : $storage_path rounds*****"
while [ $aging_cnt -gt 0 ]; do
  camera_service_aging
  cs_log "*****CS_AGING: $aging_cnt SUCCESS for 5 minutes*****"
  #sleep 10
  let "aging_cnt -= 1"
  check_camera_af_aging_result
done

# disable lens drop step cehck
cam_dt_cmdline -e camdev-lens_drop_check_enable enable=0
# turn lens PI OFF
cam_dt_cmdline -r -e lens-lens_PI_enable enable=0
