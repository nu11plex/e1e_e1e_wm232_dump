...
cs_aging_log=/data/dji/amt/aging_test/udp_cs_aging.txt
...
camera_service_aging() {
  ...
  while [ $record_time -lt $cs_rec_time ]; do
    ...
    cs_send_msg_0 70
    if [ $ack_ret -ne 0 ]; then
      cs_log "$E_CHECK_REC CS_AGING: check recording ack_ret:$ack_ret, send_cnt:$send_cnt"
      continue
    fi
    ...
    if [ $record_status -ne 2 ]; then
      check_sdcard
      cs_exit $E_CHECK_REC "CS_AGING: check recording status:$record_status ack_ret:$ack_ret"
    fi
...
if [ $# -eq 1 ]; then
  if [ $1 -lt 1800 ]; then
    cs_exit $E_INPUT "camera aging time should at least 1800 s, input time is:$1 s"
  fi
  let "aging_cnt = $1 / 1800"
fi

cs_log "*****CS_AGING: need to aging for $aging_cnt rounds*****"
while [ $aging_cnt -gt 0 ]; do
  camera_service_aging
  cs_log "*****CS_AGING: SUCCESS for 30 minutes*****"
  let "aging_cnt -= 1"
done

cs_exit 0 "CS_AGING: SUCCESS for $1 senconds!"
